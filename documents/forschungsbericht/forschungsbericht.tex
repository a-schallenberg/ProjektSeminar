...
Falls es keine Datei mit diesem Namen gibt bzw. diese keine Bilddatei ist, wird eine \texttt{FileNotFoundException} bzw. \justify{\texttt{IllegalArgumentException}} eine geworfen.
...
Das Record \texttt{TrainData(int[], int[])} besitzt einen Vektor mit den RGB-Werten eines Bildes und einen Zielvektor. Außerdem hat es zwei Instanz-Methoden \texttt{getImageAsDoubleArray()} und \texttt{getTargetAsDoubleArray()}, welche die jeweiligen ganzzahligen Vektoren als Gleitkommazahl-Vektoren zurückgibt, und eine private nicht-statische Hilfsmethode \texttt{getIntAsDoubleArray(int[])}.
